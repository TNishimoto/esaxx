...
for file in ${filearr[@]}; do
    ...
    nohup /usr/bin/time -f "#esaxx ms(rlbwt), ${file}, %e sec, %M KB" ~/esaxx/build/enumMaximalSubstringWithRLBWT.out -i ${outputpath}/${file}.rlbwt -o ${outputpath}/${file}.rlbwt.msi2 >> /mnt/server/nishimoto/result/esaxx_ms_rlbwt_output_new.log
 done
 echo "Finished."
