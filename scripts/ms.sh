...
for file in ${filearr[@]}; do
    ...
    nohup /usr/bin/time -f "#online rlbwt ${file}, %e sec, %M KB" ~/esaxx/build/online_rlbwt.out -i ${folderpath}/${file} -o ${outputpath}/${file}.rlbwt >> /mnt/server/nishimoto/result/esaxx_online_rlbwt_output.log
 done
