...
for file in ${filearr[@]}; do
    ...
    nohup /usr/bin/time -f "#esaxx ms(sdsl) ${file}, %e time, %M KB" ~/esaxx/build/enumMaximalSubstring.out -i ${folderpath}/${file} -o ${outputpath}/${file}.sdsl.msi2 -p 0 -m sdsl >> /mnt/server/nishimoto/result/esaxx_ms_sdsl_output.log
