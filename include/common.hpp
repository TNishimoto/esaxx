...
namespace stool
{
    namespace esaxx
    {
        ...
        */
        ...
        template <typename CHAR = uint8_t>
        bool check_dollar_text(std::vector<CHAR> &text)
        {
            if (text.size() == 0)
            {
                return true;
            ...
            else
            {
                ...
                if (text.size() > 1)
                {
                    for (int64_t i = 0; i < (int64_t)text.size() - 2; i++)
                    {
                        if (text[i] < dollar)
                        {
                            throw std::domain_error("Error(check_dollar_text): The last character of the input text should be smallest.");
                        }
                        else if (text[i] == dollar)
                        {
                            throw std::domain_error("Error(check_dollar_text): The last character of the input text cannot be appear twice.");
                        }
                    }
                }
                return true;
